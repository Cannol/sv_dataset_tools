...
class VideoPlayer(DatasetBase):
    ...
    def _draw(self, img, poly, rect, n, scale=1.0, state=0, show_obj=True, center_obj=False,
        ...
        search_rect_pt1 = [int(round(rect_np[0, 0] + 0.5 * (rect_np[1, 0] - rect_np[0, 0] - A))),
                           int(round(rect_np[0, 1] + 0.5 * (rect_np[1, 1] - rect_np[0, 1] - A)))]
        search_rect_pt2 = [int(round(rect_np[0, 0] + 0.5 * (rect_np[1, 0] - rect_np[0, 0] + A))),
                           int(round(rect_np[0, 1] + 0.5 * (rect_np[1, 1] - rect_np[0, 1] + A)))]
