...
---
2022.7.19 更新 AI辅助版本 测试版 V2.0.0 beta
- 【AI辅助跟踪】按t/y来决定向前还是向后进行跟踪，如果前面/后面有关键帧，则会跟踪到那个关键帧停止，如果没有关键帧，则会向前/后跟踪10帧长度
- 【跨目标合并】选择目标后，按u进入合并选择状态，选择另一个目标后，按回车确认合并
- 【批量删除】按o/p，向前或向后删除所有帧（不包括当前帧）
- 【取消某段AI跟踪结果】先位于该段结果中的任意一帧，按k键，该段将会被恢复未传统均匀补间
- 【全屏显示】按l键，切换进入全屏或非全屏状态，如果要使用全屏功能，最好将区域尺寸SelectArea设置在显示器分辨率以上大小，否则可能出现图行扭曲或残影影响标注
- 【显示优化】按n和m取消或者显示当前帧中心点和前后中心点
- 【操作优化】按e键，切换缓动微调模式，在缓动模式下，鼠标拖动目标时会发生较小位移
- 【基础优化】设置图片缓冲区，加快临近帧图片加载速度，优化大图打开体验

---
2022.11.6 更新 AI輔助版本 正式版 V2.0.1r
- 【AI辅助跟踪bug修复】修复了自动跟踪算法导致无法再标注该目标的问题，修复了freeze参数被误写入meta文件的问题
- 【新增一键生成voc数据集功能】在主界面按p，输入保存根目录后，即可将当前选区中的目标生成voc格式的数据集
...
MaxCacheMemory: 500  # 主程序的最大缓存，单位MB
```
...
- Delete或-：先选择某个目标，然后按该按键可以删除目标。删除时会有提示确认是否删除，删除后的目标无法恢复，请谨慎操作。
