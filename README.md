...
---
2022.5.31 更新 正式版本补充版本 V1.2.2 release
- 【显示优化】按b可以切换是否显示当前帧不存在的目标（黑色的框）
- 【显示优化】按c可以切换是否显示当前的选择框（不显示当前被选择的目标框时，只显示矩形框的4个操作点）
- 【显示优化】按空格键切换solo模式，当处于solo模式下，在选中某个目标的时候，仅显示该目标的框，其他目标均不可见
- 【显示优化】按q/w可以快速跳转到已选中目标的上一关键帧/下一关键帧
